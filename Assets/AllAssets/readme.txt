...
Just move them around on the game board. Their initial position isn't stored anywhere. They just move to the center of the nearest hex at the start of the game. To illustrate this, the cubes are actually not quite in the right place initially, but they're fixed when the game starts.

Unfortunately, that does not apply to the big grey blob. That thing's hard-coded. You can change the model if you want it to look different. If you want to remove it entirely, then remove line 108 of HexGrid.cs which should say 'new HexPosition().flag("Obstacle");'.
...
The obstacle is currently hard-coded into HexGrid.start(). If you want to add or remove obstacles, change them there. The prolate spheroid that actually shows where it is has nothing to do with the code and is placed separately.
